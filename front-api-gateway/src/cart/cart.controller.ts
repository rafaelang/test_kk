...
export class CartController {
  ...
  @ApiResponse({
    status: 200,
    description: 'User cart details returned successfully.',
    type: CartDto,
  })
  ...
  async addItem(
    @Param() params: GetCartIdParamDto,
    @Body() item: CartRequestOperationDto,
  ) {
    const payload: CartOperationDto = {
    ...
    try {
      ...
      if (error.exception_type === 'InvalidShoppingCartIdError') {
        throw new BadRequestException(
          `ShoppingCartId is invalid: ${error.message}`,
        );
      }
      throw error;
    }
