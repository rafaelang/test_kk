...
export class CartDao {
  ...
  async get(shoppingCartId: number): Promise<Cart | null> {
    let cart = await this.repository.findOneBy({
...
export class CartProductDao {
  ...
  async get(
    ...
    let cart_product = await this.repository.findOneBy({
      productId: productId,
      cart: { shoppingCartId: shoppingCartId, userId: userId },
    });
    return cart_product;
  }
    ...
    try {
      ...
      if (
        error.driverError.detail.includes(
          `Key (cartShoppingCartId)=(${cartProduct.cart.shoppingCartId}) is not present in table`,
        )
      ) {
        const error = new Error(
          `Invalid shoppingCartId: ${cartProduct.cart.shoppingCartId} does not exist.`,
        );
  ...
  async updateOrCreate(
    ...
    let [created, cart_product] = await this.getOrCreate(
      userId,
      cartId,
      productId,
      item,
  ...
  async getOrCreate(
    ...
    let cart_product = await this.repository.findOneBy({
      productId: productId,
      cart: { userId: userId, shoppingCartId: shoppingCartId },
    });
    let created = false;

    if (!cart_product) {
      cart_product = this.repository.create({
